...
class TrOCRAttention(nn.Module):
    ...
    def forward(
        ...
        if attention_mask is not None:
            ...
            attn_weights = attn_weights.view(bsz, self.num_heads, tgt_len, src_len) + attention_mask  # causla在这里面加注意力
...
class TrOCRDecoder(TrOCRPreTrainedModel):
    ...
    def forward(
        ...
        # past_key_values_length  # 记录之前的.
        past_key_values_length = past_key_values[0][0].shape[2] if past_key_values is not None else 0

        if inputs_embeds is None:# embedding 层. 就是一个linear
            inputs_embeds = self.embed_tokens(input_ids) * self.embed_scale

        if self.config.use_learned_position_embeddings: # 是否使用学习化的位置编码
            embed_pos = self.embed_positions(input_shape, past_key_values_length=past_key_values_length)
        else:  # 不学习的就是sin cos结构.
