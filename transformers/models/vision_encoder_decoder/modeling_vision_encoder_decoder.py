...
def shift_tokens_right(input_ids: torch.Tensor, pad_token_id: int, decoder_start_token_id: int):
    ...
    shifted_input_ids[:, 1:] = input_ids[:, :-1].clone() # 向右偏移一个单位进行赋值.之前的最后一个不要了.
...
# 进行label转化为decoder_input_ids, 这个是解码时候的输入. 懂gpt的都知道. 每一次网络输入上一次的输出.吐出下一个输出.比如现在是 0 1 2 那么网络喂入0吐出1.所以向右偏移一个就得到了全部需要喂入的东西.
